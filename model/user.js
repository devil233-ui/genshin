...
export default class User extends base {
  ...
  async delCk() {
    ...
    if (!uidData || uidData.type !== "ck" || !uidData.ltuid) {
      return `删除失败：当前的UID ${uidData?.uid || "未知"} 无CK信息`
    }
  ...
  /** 彻底修复了祖传拼写错误并传入正确的游戏前缀 */
  async bindUid() {
    let game = this.e.game || "gs"
    let reg = /(18|[1-9])[0-9]{8}/g // 默认原神、星铁
    if (game === "zzz") reg = /(1[0-9]|[1-9])[0-9]{8}|[1-9][0-9]{7}/g
    if (game === "bh3") reg = /[1-9][0-9]{7,8}/g
    ...
    // 【核心修复】必须传入字符串 game，传入 this.e 会导致 NoteUser 底层无法识别从而默认存进原神！
  ...
  async delUid() {
    ...
    if (index > uidList.length || index < 1) {
      return await this.e.reply([
        "uid序号输入错误",
        segment.button([{ text: "删除uid", input: `${prefix}删除uid` }]),
      ])
    }
  ...
  async toggleUid(index) {
    ...
    if (index > uidList.length) {
      return await this.e.reply([
        "uid序号输入错误",
        segment.button([{ text: "切换uid", input: `${prefix}uid` }]),
      ])
    }
