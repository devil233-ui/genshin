...
export default class ExploreExport {
  ...
  async run(data) {
    ...
    if (!cfg.exploreSmtpHost || !cfg.exploreSmtpUser || !cfg.exploreSmtpPass) {
      logger.warn(
        "[探索导出] SMTP 配置不完整（服务器/账号/授权码），跳过。请在锅巴面板「探索度表格邮件」中配置",
      )
      return false
    }
  ...
  async genXlsx(rows, data, cfg = {}) {
    ...
    try {
      ...
      logger.error(
        "[探索导出] 未安装 exceljs，无法生成表格。请在 Yunzai 根目录执行：pnpm add exceljs",
      )
      return ""
    }
  ...
  async sendMail(filePath, data, cfg) {
    ...
    try {
      ...
      logger.error(
        "[探索导出] 未安装 nodemailer，无法发送邮件。请在 Yunzai 根目录执行：pnpm add nodemailer",
      )
      return false
    }
