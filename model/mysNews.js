...
export default class MysNews extends base {
  ...
  async ActivityPush() {
    ...
    let allGames = [
      ...
      pushGroupList.bbbActivityPush,
      pushGroupList.zzzActivityPush,
    ]
    ...
    for (let botId of BotidList) {
      ...
      if (
        ...
        this.e &&
        this.e.bot &&
        (this.e.bot.uin == botId || this.e.bot.id == botId)
      ) {
      ...
      for (let groupId of groupList) {
        ...
        for (let a of ActivityList) {
          ...
          let isGs =
            pushGroupList.gsActivityPush?.[botId]?.map(String).includes(groupStr) && a.game === "gs"
          let isSr =
            pushGroupList.srActivityPush?.[botId]?.map(String).includes(groupStr) && a.game === "sr"
          let isBbb =
            pushGroupList.bbbActivityPush?.[botId]?.map(String).includes(groupStr) &&
            a.game === "bbb"
          let isZzz =
            pushGroupList.zzzActivityPush?.[botId]?.map(String).includes(groupStr) &&
            a.game === "zzz"
          ...
          logger.mark(
            `[米游社活动到期推送] 开始推送 ${botId}:${groupId} [${pushGame}] ${a.subtitle}`,
          )
          pushSuccessCount++
          ...
          botInstance
            ...
            .sendMsg(msgList)
            .then(() => {})
            .catch(err => logger.error(`[米游社活动到期推送] 推送失败，错误信息${err}`))
  ...
  async getGsActivity() {
    ...
    for (let item of gshd.data.list[1].list) {
      if (
        item.type_label.includes("活动公告") &&
        !item.title?.includes("冒险助力礼包") &&
        !item.title?.includes("七圣")
      )
        hdlist.push(item)
    }
  ...
  async getSrActivity() {
    ...
    for (let item of srhd.data.pic_list[0].type_list[0].list) {
      if (
        ...
        (item.type_label?.includes("资讯") &&
          !item.title?.includes("签到") &&
          !item.title?.includes("PV"))
      )
        hdlist.push(item)
    }
    for (let item of hdlist) {
  ...
  async getZzzActivity() {
    ...
    try {
      zzzhd = await fetch(
        "https://announcement-api.mihoyo.com/common/nap_cn/announcement/api/getAnnList?game=nap&game_biz=nap_cn&lang=zh-cn&bundle_id=nap_cn&platform=pc&region=prod_gf_cn&level=60&uid=100000000",
      )
    ...
    for (let item of zzzhd.data.pic_list[0].type_list[0].list) {
      if (
        item.title?.includes("活动") ||
        (item.type_label?.includes("资讯") &&
          !item.title?.includes("主线") &&
          !item.title?.includes("上新"))
      )
        hdlist.push(item)
    }
    for (let item of hdlist) {
    ...
    if (bbbhd?.data?.list) {
      for (let lst of bbbhd.data.list) {
        ...
        for (let item of lst.list) {
          if (
            item.type_label?.includes("活动") &&
            !item.title?.includes("补给") &&
            !item.title?.includes("跃升武装") &&
            !item.title?.includes("礼包") &&
            !item.title?.includes("凭证") &&
            !item.title?.includes("七日登录")
          ) {
