...
const buildSchemas = () => {
  ...
  schemas.push({
    ...
    label: "显示聚所/特殊供奉",
    bottomHelpMessage:
      "开启后在新版探索度卡片中显示「聚所/特殊供奉」区块（如挪德卡莱的8个聚所）。仅在新版样式下生效",
    component: "Switch",
  })
  ...
  schemas.push({
    ...
    label: "显示详细区域探索度",
    bottomHelpMessage:
      "开启后在新版探索度卡片中显示「详细区域探索度」区块（如璃月的碧水原、珉林等细分区域列表）。仅在新版样式下生效",
    component: "Switch",
  })
  ...
  schemas.push({
    ...
    label: "显示区域首领图鉴",
    bottomHelpMessage:
      "开启后在新版探索度卡片中显示「区域首领图鉴」区块（已击杀次数）。仅在新版样式下生效",
    component: "Switch",
  })
  ...
  schemas.push({
    ...
    label: "提示文字",
    bottomHelpMessage: "附加在「即将发送邮件」提示与邮件正文末尾的说明文字。留空则不显示",
    component: "Input",
    componentProps: {
      type: "textarea",
      rows: 2,
  ...
  schemas.push({
    ...
    label: "距离达峰 列名",
    bottomHelpMessage:
      "xlsx 表格中差值列的表头，该列为 0 时显示绿色、不为 0 显示红色。留空则用默认值「距离达峰」",
    component: "Input",
    ...
    configInfo: {
      ...
      getConfigData() {
        ...
        data.exploreMailNotice =
          set.exploreMailNotice ??
          "# 注意：可给机器人发送  #关闭邮件发送  、#开启邮件发送  控制是否发送邮件"
      ...
      setConfigData(data, { Result }) {
        ...
        for (const [keyPath, val] of Object.entries(data)) {
          ...
          // 跳过未知字段
          if (!GROUP_KEYS.includes(keyPath) && !["pushTime", "maxNum"].includes(keyPath)) continue
